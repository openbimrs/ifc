...
PUBLISHED_STATUS = {
    ...
    # IFC4 and IFC4X3 construction-resource occurrences, resource types,
    # actors (person/organization/role), inventory, authored usage time,
    # usage quantities, allocation, bounded composition, and selected
    # transaction-staged authoring are tested (identical entity shapes
    # verified against IFC4.exp/IFC4X3_ADD2.exp). IFC2X3 does not declare
    # IfcConstructionResourceType, IfcResourceTime, or PredefinedType on two
    # occurrence subtypes, so it is a typed UnsupportedSchema refusal rather
    # than a gap.
    ...
    # bounded relationships; selected model/load/member/connection/action and
    # relationship authoring is transaction-staged. Facade composition with
    # physical products/geometry uses shared EntityId only. Solving, FEM,
...
def unhandled_table() -> str:
    ...
    if not items:
        # The missing-schema case already raised above, so an empty list here
        # is a measurement, not an absence of one: every concrete geometry
        # item is now named somewhere in the source. Saying "schema was not
        # available" would under-claim as dishonestly as the old tables
        # over-claimed.
        return (
            "Every concrete `IfcRepresentationItem` subtype in IFC4 ADD2 TC1 is "
            "named somewhere in this repository -- implemented, refused, or "
            "explicitly classified as carrying no shape. This section is "
            "generated by walking down from the schema, so it reports zero only "
            "when the walk actually finds nothing unaddressed. Being *named* is "
            "not the same as being implemented: see the tables above for the "
            "status of each one."
        )
    lines = [
