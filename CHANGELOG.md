...
- `ifc-schema` can export deterministic, release-identified structural catalogs
  ...
  without redistributing EXPRESS sources or specification prose. A direct mode
  emits only each entity's immediate parent and declared slots; reconstruction
  tests prove it expands byte-for-byte to every release's full structural rows.
