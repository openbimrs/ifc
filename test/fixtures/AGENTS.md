...
Small `.ifc` files for use in crate tests, copied from the reference repos
under `references/` (real trees on `/mnt/backup/`). Total ~380 KiB, 18 files —
...
1. **These are data, not source** — copying them out of `references/` is fine
   despite the upstream licenses (LGPL/MPL restrict source redistribution in
   derivative code, not test-fixture data files); still, don't add a fixture
   that looks like a real client project export rather than a synthetic/minimal
   repro case.
...
3. **Add new fixtures deliberately, not in bulk.** The point of this directory
   ...
   new file, check it actually exercises something the existing 18 don't.
...
5. Full oracle-scale `.ifc` corpora (e.g. Solibri's example models) belong to
   `../vendor/solibri`'s `data/oracle/`, not here — this repo's fixtures stay
