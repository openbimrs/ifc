...
## Try it: a downloadable round-trip file

[`annotation-plan.ifc`](/fixtures/annotation-plan.ifc){target="_self" download} (2.8 KB, IFC4) is one
fire-wall symbol drawn the way this page recommends:

| Entity | In the file |
| --- | --- |
| `IfcAnnotation` | `Brandwand`, contained in storey `EG` |
| `IfcShapeRepresentation` | `Annotation2D`, in a `PLAN_VIEW` `Annotation` sub-context at 1:100 |
| `IfcPolyline` | the wall line, 6000 mm |
| `IfcTextLiteralWithExtent` | `Brandwand F90`, a 2000 × 300 mm box, `bottom-left` |
| `IfcCurveStyle`, `IfcTextStyle`, `IfcStyledItem` | red, 50 mm line; Arial 250 mm text |
| `IfcPresentationLayerAssignment` | layer `A-ANNO-FIRE` |
| `IfcLibraryReference`, `IfcRelAssociatesLibrary` | symbol `BW-F90` in library `Brandschutz-Symbole` |
...
What the file does **not** show: there is no `IfcApproval`, because approvals
have no typed view yet (see the table below), and nothing here renders. The
styles are read back as data; drawing them is your application's job.

The file is synthetic and generated by
[`tools/gen_annotation_fixture.py`](https://github.com/openbimrs/ifc/blob/main/tools/gen_annotation_fixture.py)
with IfcOpenShell, which validates it (including EXPRESS rules) before
writing. It contains no third-party or normative material, and is licensed
with the rest of the repository under AGPL-3.0-or-later.
