...
| `ifc-alignment` | 2,869 | 29 | 11 | 5 | <span class="status-partial">Partial</span> |
...
| `ifc-spatial` | 1,045 | 9 | 1 | 8 | <span class="status-implemented">Implemented</span> |
...
| Objectified relationship traversal | <span class="status-partial">Partial</span> | 37 of the schema's 42 concrete `IfcRel*` families are read, across 14 crates. Spatial composition, containment, coverings, element connection and interference in `ifc-spatial`; assignment, association and definition in `ifc-classification`, `ifc-material`, `ifc-properties`, `ifc-resource`, `ifc-approval` and `ifc-constraint`; connectivity in `ifc-systems` and `ifc-structural`; sequencing in `ifc-schedule` and `ifc-cost`; feature attachment in the `openbim-ifc` reachability check. Space boundaries are read across all three concrete types (`IfcRelSpaceBoundary`, `1stLevel`, `2ndLevel`) in `ifc-spatial::relation::boundary`, with `ParentBoundary`/`CorrespondingBoundary` links and the `CorrectPhysOrVirt` agreement reported rather than enforced. The 5 unread families are listed in `openbim-ifc/tests/relationship_census.rs`, which fails when this count drifts from the source. |
...
| `IfcPcurve` | <span class="status-partial">Partial</span> — 2 authored form(s) refused; see [variants](#partially-supported-variants) |
