...
| `ifc-spatial` | 908 | 9 | 1 | 6 | <span class="status-implemented">Implemented</span> |
...
| Objectified relationship traversal | <span class="status-partial">Partial</span> | 29 of the schema's 42 concrete `IfcRel*` families are read, across 14 crates. Spatial composition, containment and coverings in `ifc-spatial`; assignment, association and definition in `ifc-classification`, `ifc-material`, `ifc-properties`, `ifc-resource`, `ifc-approval` and `ifc-constraint`; connectivity in `ifc-systems` and `ifc-structural`; sequencing in `ifc-schedule` and `ifc-cost`; feature attachment in the `openbim-ifc` reachability check. Space boundaries are read across all three concrete types (`IfcRelSpaceBoundary`, `1stLevel`, `2ndLevel`) in `ifc-spatial::relation::boundary`, with `ParentBoundary`/`CorrespondingBoundary` links and the `CorrectPhysOrVirt` agreement reported rather than enforced. The 13 unread families are listed in `openbim-ifc/tests/relationship_census.rs`, which fails when this count drifts from the source. |
