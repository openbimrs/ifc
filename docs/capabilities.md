...
| `ifc-alignment` | 4,033 | 35 | 11 | 7 | <span class="status-partial">Partial</span> |
...
| `ifc-schedule` | 1,926 | 25 | 13 | 2 | <span class="status-implemented">Implemented</span> |
...
Every concrete `IfcRepresentationItem` subtype in IFC4 ADD2 TC1 is named somewhere in this repository -- implemented, refused, or explicitly classified as carrying no shape. This section is generated by walking down from the schema, so it reports zero only when the walk actually finds nothing unaddressed. Being *named* is not the same as being implemented: see the tables above for the status of each one.
