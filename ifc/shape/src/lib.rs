//! `ifc-shape` — lower IFC representation items into geometry.
//!
...
//!
//! ```text
//!   ifc-parser ─→ ifc-model ─→ ifc-shape ──uses trait──→ geom-kernel
//!                                                             ▲
...
//! [`geom_core::TriMesh`], with boolean operations delegated to the injected
...
mod tests {
    ...
    /// crate, and `ifc-shape` accepts it without modification.
    struct StubKernel;
