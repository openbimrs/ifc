...
Applies to `packages/**`. Read the nearest deeper `AGENTS.md` before
...
## Layers inside `packages/`

The directory is flat; the layer a crate belongs to is carried by its NAME, and
the architecture tests select on that name rather than on a parent directory.

| Prefix | Layer | Rule |
| --- | --- | --- |
| `ifc-*`, `openbim-ifc` | IFC | May not depend on any `openbim-*` standard crate |
| `openbim-codec-*` | Encoding substrate | Depends on nothing in this workspace |
| `openbim-*` | openBIM standards | Consume the IFC layer, never the reverse |
| `clash`, `diff` | Analysis capabilities | Not standards; may emit `openbim-bcf` findings |
| `icdd`, `idmxml`, `loin` | Alias crates | Pure `pub use` re-exports; no types of their own |

## The one-way rule

The IFC layer must never depend on an openBIM standard crate. If an IFC crate
needs something from a standard, the abstraction is in the wrong place: move
the shared piece down into the IFC layer or into `openbim-codec-*`, never pull
the dependency up. `packages/ifc-model/tests/package_architecture.rs` enforces
this, and `ifc-geometry/tests/no_backend_dependency.rs` enforces the parallel
rule for geometry.

That is also why the XML and ZIP substrate is `openbim-codec-*` and sits below
both layers: `ifc-xml` and the standard crates both need it, and a shared piece
living inside `openbim-*` would have forced exactly the dependency the rule
forbids.
