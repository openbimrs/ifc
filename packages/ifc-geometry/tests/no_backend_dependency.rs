...
/// The IFC layer, now that `packages/` is flat: every `ifc-*` crate plus the
/// `openbim-ifc` facade. Selecting by name rather than by directory is what
/// keeps this boundary test meaningful after the layout flattened -- a
...
fn ifc_layer_crates() -> Vec<PathBuf> {
    let mut out = Vec::new();
    for entry in std::fs::read_dir(packages_dir()).expect("packages/ must exist") {
...
fn active_lowering_does_not_use_legacy_request_vocabulary() {
    ...
    let lower = packages_dir().join("ifc-geometry/src/lower");
...
fn allowlist_names_only_real_crates() {
    for allowed in MAY_USE_GEOMETRY {
        assert!(
            packages_dir().join(allowed).join("Cargo.toml").exists(),
