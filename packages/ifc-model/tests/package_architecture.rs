...
fn ifc_packages() -> BTreeMap<String, Package> {
    ...
    let packages_root = metadata.workspace_root.as_std_path().join("packages");
    metadata
        .packages
        .into_iter()
        .filter_map(|package| {
            let crate_dir = package.manifest_path.as_std_path().parent()?;
            let under_packages = crate_dir.parent() == Some(packages_root.as_path());
            let is_ifc_layer = package.name.starts_with("ifc-") || package.name == FACADE;
            (under_packages && is_ifc_layer).then(|| (package.name.to_string(), package))
        })
        .collect()
}
