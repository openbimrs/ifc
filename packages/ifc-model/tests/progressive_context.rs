...
///
/// `packages/` is flat, so the IFC layer is selected by crate-directory NAME.
/// The openBIM standard crates live beside these and follow their own
/// documentation rules, so sweeping them in here would be wrong.
