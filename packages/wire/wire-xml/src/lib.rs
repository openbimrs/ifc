//! `wire-xml` — container-level XML recognition.
//!
//! # Why this is its own crate
//!
//! Both `packages/ifc/` (ifcXML) and `packages/openbim/` (BCF, IDS, IDM, LOIN,
//! ICDD) read XML. The workspace rule is that `packages/ifc/` must never depend
//! on `packages/openbim/`, so a shared XML layer cannot live inside `openbim`.
...
///
/// ```
/// use wire_xml::{strip_bom, UTF8_BOM};
