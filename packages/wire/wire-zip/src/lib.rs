//! `wire-zip` — ZIP container recognition for openBIM archive formats.
//!
...
//! `packages/ifc/` and `packages/openbim/`, because IFCZIP is an IFC concern
//! and the other two are not.
//!
//! It is separate from `wire-xml` so that a consumer reading a plain `.ids`
