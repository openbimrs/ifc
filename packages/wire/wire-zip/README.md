...
BCF, ICDD and IFCZIP all ship as ZIP archives and share nothing else. Recognising that envelope belongs in one place. Kept separate from `wire-xml` so that reading a plain `.ids` file never links a ZIP implementation.
